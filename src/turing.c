...
static uint16_t random[] = { 2, 4, 0xffff, 8, 16   };
static uint16_t length[] = { 2, 3, 4, 5, 8, 12, 16 };
static uint16_t range[]  = { 7, 6, 5, 4, 3         };
...
{
  return length[t->idx_len];
}

uint8_t turing_clock(turing_t *t)
{
  uint16_t mask = 1 << (length[t->idx_len] - 1);
  uint16_t top = t->shiftreg & mask;

  if (random[t->idx_rand] != 0xffff && ((rand(t) & (random[t->idx_rand] - 1)) == 0)) {
  ...
  t->shiftreg = (top >> (length[t->idx_len] - 1)) | (t->shiftreg << 1);

  return (t->shiftreg & 0x00ff) >> range[t->idx_range];
}
